...
##### Calcul des indicateurs pour une zone

Modifier le code en fonction des quartiers utilisés précédemment. 

Les identifiants des zones permettent de réaliser plus tard une jointure pour récupérer les données.

```{r}
dataset_grouped <- dataset |> 
  mutate(
    zone = case_when(
      id %in% quartier_ids[[1]] ~ min(quartier_ids[[1]]),
      id %in% quartier_ids[[2]] ~ min(quartier_ids[[2]]),
      id %in% quartier_ids[[3]] ~ min(quartier_ids[[3]]),
      TRUE ~ id
    )
  ) |> 
  group_by(zone) |> 
...
leaflet() |>
  ...
  addPolygons(
    data = city_hex_l93_intersection_dataset_souspop2, # modifiez avec les données à utiliser
    color = "#000",

...
dataset <- data.frame(
  ...
  critere_bien_etre_global = round(runif(nrow(creteil_square_l93), min = 0, max = 5), digits = 2),
  critere_type_nature_global = round(runif(nrow(creteil_square_l93), min = 0, max = 5), digits = 2),
  critere_biodiv = round(runif(nrow(creteil_square_l93), min = 0, max = 5), digits = 2),
  critere_exposition_nature = round(runif(nrow(creteil_square_l93), min = 0, max = 5), digits = 2),
  critere_activ_global = round(runif(nrow(creteil_square_l93), min = 0, max = 5), digits = 2),
  critere_caract_percues = round(runif(nrow(creteil_square_l93), min = 0, max = 5), digits = 2)
)
...
pal <- colorBin(
  "Greens",
  domain = creteil_square_data_wgs84$critere_bien_etre_global, 
  bins = bins,
  na.color  = "transparent"
)
...
leaflet() |> 
  ...
  addPolygons(
    ...
    color = "#000000", 
    fillColor = ~pal(critere_bien_etre_global),
    weight = 1, 
    opacity = 0.2, 
    fillOpacity = 0.4,
    label = ~critere_bien_etre_global
  ) %>%
  addLegend(
    pal = pal,
    values = creteil_square_data_wgs84_inter$critere_bien_etre_global,
    title = "Note moyenne",

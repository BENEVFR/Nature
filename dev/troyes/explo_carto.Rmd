...
communes_l93 |> 
  st_drop_geometry() |> 
  filter(grepl("Troyes", NOM))

# Récupérer le polygone de votre commune
troyes_l93 <- communes_l93 |> 
  filter(NOM == "Troyes")
```
...
```{r}
st_write(troyes_l93, dsn = here::here("dev/troyes_l93.gpkg"))

# On peut imaginer commencer le script ici
troyes_l93 <- st_read(here::here("dev/troyes_l93.gpkg"))
```
...
troyes_wgs84 <- troyes_l93 |> st_transform(crs = 4326) # EPSG: 4326

leaflet(troyes_wgs84) |> 
  addTiles() |> 
  addPolygons()
```
...
```{r}
troyes_square_l93 <- troyes_l93 %>% 
  st_make_grid(
    what = "polygons", 
    square = TRUE, # TRUE = rectangle, FALSE = hexagones
    cellsize = c(200, 200) # Dimension en mètre, car shapefile en mètre (lambert 93)
  ) %>% 
  st_sf() %>% 
  mutate(id = 1:n()) %>% 
...
```{r}
# La carte doit être en coordonnées non projetées
troyes_square_wgs84 <- troyes_square_l93 |> st_transform(crs = 4326)
troyes_square_centroid_wgs84 <- troyes_square_wgs84 |> st_centroid()

leaflet() |> 
  addTiles() |> 
  addPolygons(data = troyes_square_wgs84,
              color = "#ff0000", fillColor = "transparent",
              weight = 1) |> 
  addPolygons(data = troyes_wgs84) |> 

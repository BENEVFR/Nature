...
```{r}
# Lecture des vraies données de l'enquête 
# + 
# agrégation avec la fonction calculate_indicators
data <- readxl::read_excel("verneuil_data.xlsx") |>
...
city_hex_l93_intersection_dataset <- city_hex_l93_intersection |> 
  ...
  mutate(
    across(
      starts_with("critere") & -critere_ratio_bien_etre_nature,
      ~ my_palette(.x),
      .names = "{.col}_color"
    ),
    across(
